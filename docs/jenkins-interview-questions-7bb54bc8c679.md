...
Cron 语法用五个星号表示，每个星号用一个空格隔开。语法如下—[分钟][小时][一个月中的某一天][月][一周中的某一天]。例如，如果您想为每个星期一晚上 11:59 设置一个 cron，那么应该是 59 11 * * 1
...
要使用管道作为代码，项目必须在存储库根目录中包含一个名为`Jenkinsfile`的文件，该文件包含一个“管道脚本”
